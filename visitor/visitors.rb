...
class Visitor
  def self.visit(element)
    send(:"visit_#{element.class.name.downcase}", element)
  end
end
...
class PackagerVisitor < Visitor
  ...
  def self.visit_dinnerware(products)
    puts "Packaging the dinnerware (complect of #{products.color} " \
    "#{products.type} - #{products.number_of_pieces} objects) in the carton " \
    'box...'
    pay_off products
  end
...
class AssistantVisitor < Visitor
  def self.visit_weapon(weapon)
    puts 'You see this? This... is my BOOMSTICK! ' \
      ...
      "#{weapon.price}. It's got a #{weapon.stock_material} stock, " \
      "#{weapon.material}."
    say_slogan
  end
  ...
  def self.visit_dinnerware(products)
    puts 'Well hello Mister Fancypants. ' \
      "What are you going to buy is just a bunch of #{products.color} " \
      "#{products.type}, retail price is #{products.price_per_product} for a " \
      'single... thing. If you can\'t do the math - go ahead and run. Run ' \
      'home and cry to mama! Ummm... Anyway, the full price is ' \
      "#{products.full_price}$, so... Come get some."
    say_slogan
  end
